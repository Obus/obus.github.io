...
Этот метод решает задачу кластеризации --- разбиения множества точек $$\{x_i\}_1^N$$ на $$K$$ ($$\leq N$$) непересекающихся множеств-кластеров.  Результат кластеризации методом $K$-средних --- это, во-первых, разбиение множества $\{1,\ldots,N\}$ на $$K$$ кластеров: $$\{\mathcal{C}_k\}_1^K$$ и, во-вторых, центр каждого из этих кластеров: $$\{\mu_k\}_1^K$$. Функция $$\risk(\theta)$$ для метода $K$-средних записывается следующим образом
...
Этот метод решает задачу регрессии --- предсказания выходов $$y_i\in\mathbb{R}$$ по входам $$x_i=(x_i^{(1)},\ldots,x_i^{(p)})^T\in\mathbb{R}^p$$ на основе обучающей выборки $$\{ x_i, y_i \}_1^N$$. Метод линейной регрессии "предполагает", что выход $$y$$ может быть представлен в виде линейной функции от входов $$x_i^{(1)},\ldots,x_i^{(p)}$$ и от параметров $$\theta^{(1)},\ldots,\theta^{(p)}$$: $$y = \sum_{j=1}^p \theta^{(j)} x^{(j)}$$. $$\risk(\theta)$$ записывается следующим образом:
